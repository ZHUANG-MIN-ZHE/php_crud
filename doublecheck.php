...
if (isset($_POST['Confirm'])) {
	$sqlcmd='INSERT INTO account_info (帳號,姓名,性別,生日,信箱,備註) VALUES ('
            . ":account,:name,:gender,:birthday,:email,:text)";
	$exec = [':account' => $_SESSION['account'], ':name' => $_SESSION['name'], ':gender' => $_SESSION['gender'], ':birthday' => $_SESSION['birthday'], ':email' => $_SESSION['email'], ':text' => $_SESSION['text']];
	// $stmt = $db_conn->prepare($sqlcmd);
	// $stmt->execute([':account' => $_SESSION['account'], ':name' => $_SESSION['name'], ':gender' => $_SESSION['gender'], ':birthday' => $_SESSION['birthday'], ':email' => $_SESSION['email'], ':text' => $_SESSION['text']]);
	$db->inserttable($db_conn, $sqlcmd, $exec);
	...
	<div>
		<form method="POST" action="" align="center">
			<table align="center">
				<tr>
					<th>帳號</th>
					<td><input type="text" name="Account" size="30" value="<?php echo $_SESSION['account']; ?>"></td>
				</tr>
				<tr>
					<th>姓名</th>
